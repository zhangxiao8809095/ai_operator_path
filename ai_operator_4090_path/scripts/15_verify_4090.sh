...
echo "[1/5] Static upload check"
"$PYTHON_BIN" scripts/verify_workspace.py --allow-generated

echo "[2/5] RTX 4090 and CUDA toolchain check"
bash scripts/00_check_env.sh --strict

echo "[3/5] Clean extension build for sm_89"
bash scripts/clean_build.sh
bash scripts/10_build.sh

echo "[4/5] Extension, 24 exports and sm_89 cubin preflight"
