...
后续Gate 1～Gate 5不是把同一批测试重复五遍，而是逐层回答“结果是否可信 → 底层执行是否合法 → 实际快慢是否稳定 → 时间消耗在哪一层 → kernel内部为什么快或慢”。前一Gate失败时停止后续重型实验，先完成定位、修复和回归。

#### Gate 1目的：基础正确性

| 字段             | 内容                                                     |
| ---------------- | -------------------------------------------------------- |
| 核心目的         | 证明接口契约、数值结果和数学性质可信                     |
| 本阶段回答的问题 | 结果是否算对；边界、特殊数值和非默认stream是否正确       |
| 为何按此顺序执行 | 错误结果没有性能分析价值；失败时立即停止后续重型实验     |
| 主要产物         | pytest日志、误差与首个错误位置                           |

#### Gate 2目的：Compute Sanitizer

| 字段             | 内容                                                     |
| ---------------- | -------------------------------------------------------- |
| 核心目的         | 证明kernel不存在未解释的内存、竞态、初始化和同步错误     |
| 本阶段回答的问题 | 结果偶尔正确时，底层执行是否合法且稳定                   |
| 为何按此顺序执行 | 普通对拍不一定触发或暴露越界、race和同步违规             |
| 主要产物         | memcheck、racecheck、initcheck、synccheck日志            |

#### Gate 3目的：Benchmark

| 字段             | 内容                                                       |
| ---------------- | ---------------------------------------------------------- |
| 核心目的         | 建立可复现的实际快慢基线，并判断波动是否可接受             |
| 本阶段回答的问题 | 固定工作量下谁更快；差异是否超过测量噪声                   |
| 为何按此顺序执行 | 先确认结果与执行安全，再用多轮采样决定哪些版本值得深入分析 |
| 主要产物         | median、P90、min/max、spread及CSV                          |

#### Gate 4目的：NSYS归层

| 字段             | 内容                                                         |
| ---------------- | ------------------------------------------------------------ |
| 核心目的         | 把端到端时间拆到准备、转换、分配、拷贝、同步、launch和kernel |
| 本阶段回答的问题 | 慢在kernel内部，还是慢在kernel之外                           |
| 为何按此顺序执行 | 只有确认目标kernel确实占据关键时间，NCU微架构分析才有价值    |
| 主要产物         | NSYS报告、时间归层结论和目标kernel                           |

#### Gate 5目的：NCU

| 字段             | 内容                                                       |
| ---------------- | ---------------------------------------------------------- |
| 核心目的         | 用固定八指标和专项指标解释kernel内部快慢的硬件原因         |
| 本阶段回答的问题 | 瓶颈属于计算、访存、资源限制还是warp调度等待               |
| 为何按此顺序执行 | 这是成本最高、最细粒度的分析，必须以前四层证据确定分析对象 |
| 主要产物         | NCU报告、八指标表、补充指标与根因结论                      |
