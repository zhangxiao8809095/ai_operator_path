...
> 版本：2.3，正常掌握、自包含教材、概念定义与关系双维、章内自查答案版。
...
下面逐组定义总表中的基础概念。定义只回答“这个词是什么”；怎样从代码变化一路影响 Duration，由后面的性能关系地图回答。

#### 概念组 1：实验契约

| 基础概念 | 定义                                                                                 |
| :------- | :----------------------------------------------------------------------------------- |
| Shape    | 输入和输出各逻辑维度的长度组合，也是决定并行规模、边界和工作量的实验条件。           |
| Dtype    | 元素的存储与解释类型；它决定数据宽度、可表示范围、精度和可选硬件路径。               |
| Layout   | 逻辑坐标到物理地址的排列规则，包括维度顺序、Stride、打包方式和对齐。                 |
| 精度     | 实现采用的输入表示、运算与累加类型、输出类型以及允许误差的完整数值约定。             |
| Baseline | 在同一契约和测量协议下作为比较基准的已知实现，例如数学 Reference、Naive 或供应商库。 |
| 统计协议 | 对 Warmup、重复次数、同步、统计量、异常值和运行环境作出的统一测量规定。              |

#### 概念组 2：有效工作量

| 基础概念        | 定义                                                                                   |
| :-------------- | :------------------------------------------------------------------------------------- |
| Effective FLOPs | 按目标数学任务定义的有效浮点运算次数，不包含实现额外执行的无效、重复或转换指令。       |
| 最小 Bytes      | 在理想复用且不计事务浪费时，为完成数学任务不可避免要跨指定存储层级搬运的理论字节下界。 |
| 实际指令        | 编译后硬件真正执行的指令，包括地址、控制、转换、无效 Tile 和数据搬运等实现成本。       |
| 实际传输        | Profiler 在明确的 L1、L2、DRAM 等层级测得的真实请求、事务或字节数量。                  |

#### 概念组 3：结果指标

| 基础概念   | 定义                                                                               |
| :--------- | :--------------------------------------------------------------------------------- |
| Duration   | 被明确起止点包围的一段实际执行时间，例如某个 Kernel 的设备执行时长。               |
| Latency    | 一个操作或请求从开始到完成所经历的时间，可包含 Launch、排队、同步和多个 Kernel。   |
| Throughput | 单位时间完成的请求、Token、样本、元素或其他有效工作数量。                          |
| TFLOP/s    | 每秒完成的万亿次浮点运算；优化报告中应明确分子采用 Effective 还是 Executed FLOPs。 |
| 有效 GB/s  | 用算法约定的有效读写字节除以 Duration 得到的结果指标，并不等于硬件实际 DRAM 带宽。 |

#### 概念组 4：性能上限

| 基础概念             | 定义                                                                           |
| :------------------- | :----------------------------------------------------------------------------- |
| Arithmetic Intensity | 算术强度，完成任务的 FLOPs 与跨指定存储层级 Bytes 之比，单位通常为 FLOP/Byte。 |
| 峰值算力             | 特定架构、数据类型和指令路径在理想条件下每秒可执行的理论最大浮点运算数。       |
| 带宽                 | 单位时间可在两个存储或系统层级之间传输的数据量，必须说明是理论峰值还是实测值。 |
| Roofline             | 用 `min(峰值算力, 带宽×算术强度)` 给出计算与带宽共同约束下性能上限的宏观模型。 |

#### 概念组 5：并行映射

| 基础概念    | 定义                                                                               |
| :---------- | :--------------------------------------------------------------------------------- |
| Grid        | 一次 Kernel Launch 产生的全部 CTA/Block 的集合。                                   |
| CTA         | Cooperative Thread Array，即 CUDA Block：可共享片上存储并进行 CTA 级同步的线程组。 |
| Warp        | GPU 调度执行线程指令的基本线程组，当前 NVIDIA CUDA 架构通常包含 32 个 Lane。       |
| Lane        | Warp 内某个线程的位置编号，用于表示该线程在协同指令、掩码和地址序列中的角色。      |
| Wave        | 性能分析中“一批足以占用当前所有可用 SM 驻留槽位的 CTA”，不是独立的 CUDA 语法对象。 |
| Tail        | 最后一批 Wave、Warp 或 Tile 未被有效任务完全填满而产生的空闲或无效工作部分。       |
| Tile 利用率 | Tile 中实际对应合法任务的元素或运算槽位占全部启动槽位的比例。                      |

#### 概念组 6：数据复用

| 基础概念    | 定义                                                                           |
| :---------- | :----------------------------------------------------------------------------- |
| Global      | 可被 Grid 中线程访问的大容量设备全局地址空间，通常经 Cache 最终由 DRAM 承载。  |
| Shared      | 由同一 CTA 线程显式管理和共享的片上存储，用于协作加载、交换和复用数据。        |
| Register    | 主要由单线程私有持有的片上状态，适合保存累加器和高频复用的小数据。             |
| CTA Tile    | 一个 CTA 共同加载或计算的逻辑数据子块，其内容通常在 CTA 内通过 Shared 被复用。 |
| Thread Tile | 单个线程负责计算并保存在寄存器中的输出子块。                                   |

#### 概念组 7：访存效率

| 基础概念      | 定义                                                                                   |
| :------------ | :------------------------------------------------------------------------------------- |
| Coalescing    | 同一 Warp 的 Global 访存地址被硬件合并为较少内存事务的性质。                           |
| Alignment     | 地址满足某种访问宽度或硬件事务所要求的整数倍边界。                                     |
| Vectorization | 使用一条宽指令为线程处理多个相邻元素，以改变指令数和单指令数据宽度。                   |
| Sector        | Cache Line 内可独立请求或统计的较小传输区域；常见 NVIDIA Profiler 口径通常为 32 Byte。 |
| Bank          | Shared Memory 中可并行服务访问的存储分区；同一 Warp 的地址映射决定是否发生冲突。       |

#### 概念组 8：资源限制

| 基础概念         | 定义                                                                       |
| :--------------- | :------------------------------------------------------------------------- |
| Registers/thread | 编译后平均分配给每个线程的寄存器数量，会参与限制 CTA/Warp 的同时驻留数量。 |
| Shared/CTA       | 单个 CTA 静态和动态申请的 Shared Memory 总量。                             |
| Threads/CTA      | 单个 CTA 中的线程数量，既决定线程布局，也占用 SM 的线程和 Warp 驻留资源。  |
| Occupancy        | 一个 SM 上 Active Warp 数相对于该 SM 架构允许最大 Active Warp 数的比例。   |

#### 概念组 9：调度状态

| 基础概念      | 定义                                                                             |
| :------------ | :------------------------------------------------------------------------------- |
| Active Warp   | 已驻留在 SM 上、尚未执行结束并占用调度资源的 Warp。                              |
| Eligible Warp | 当前周期不存在未满足依赖或其他阻塞、已经具备发射下一条指令条件的 Active Warp。   |
| Issue Slot    | Warp Scheduler 在一个周期中可用于向执行管线发射指令的机会或容量。                |
| Stall         | Warp 因数据依赖、内存、Barrier、执行管线等原因暂时不具备发射条件的等待状态分类。 |

#### 概念组 10：重叠与流水

| 基础概念             | 定义                                                                               |
| :------------------- | :--------------------------------------------------------------------------------- |
| Load/Compute Overlap | 让一部分数据加载与另一部分数据计算在时间上并行，以隐藏部分搬运等待的执行方式。     |
| Stage                | 软件流水线中的一个处理阶段或一套缓冲区，通常对应预取、等待、计算等不同进度的数据。 |
| Barrier              | 要求指定作用域内参与者都到达并满足相应内存可见性后才能继续的同步点。               |
| 依赖链               | 后一条指令必须等待前一条指令结果或资源才能执行的一系列先后关系。                   |

#### 概念组 11：低精度路径

| 基础概念        | 定义                                                                                            |
| :-------------- | :---------------------------------------------------------------------------------------------- |
| FP16            | 1 位符号、5 位指数和 10 位显式尾数的 IEEE Binary16 格式，范围较窄而精度高于 BF16。              |
| BF16            | 1 位符号、8 位指数和 7 位显式尾数的 BFloat16 格式，范围接近 FP32 而精度低于 FP16。              |
| TF32            | NVIDIA Tensor Core 的一种计算格式，通常接受 FP32 数据但使用较低乘法有效精度并进行更高精度累加。 |
| FP32 Accumulate | 低精度乘法或输入路径中使用 FP32 保存和更新中间累加结果的数值策略。                              |
| Tensor Core     | NVIDIA GPU 中用于矩阵乘加等密集运算的专用硬件执行单元。                                         |
| WMMA            | CUDA 提供的 Warp 级矩阵乘加编程接口，由一个 Warp 协同加载 Fragment 并执行 MMA。                 |

#### 概念组 12：工具证据链

| 基础概念     | 定义                                                                                       |
| :----------- | :----------------------------------------------------------------------------------------- |
| NSYS         | NVIDIA Nsight Systems，分析 CPU、CUDA API、Memcpy、同步和 Kernel 时间线的系统级 Profiler。 |
| NCU          | NVIDIA Nsight Compute，对单个或一组 GPU Kernel 收集资源、访存、调度和指令指标的 Profiler。 |
| Roofline     | 将实测或估算算术强度、算力和带宽放在同一上限图中进行宏观瓶颈判断的模型。                   |
| PTX          | CUDA 编译链中的虚拟指令集和中间表示，可再由驱动针对目标 GPU 生成机器指令。                 |
| SASS         | NVIDIA GPU 实际执行的架构相关机器指令表示。                                                |
| 编译资源报告 | 编译器输出的寄存器、Shared、Stack、Spill 等静态资源使用信息。                              |

#### 概念组 13：算子迁移

| 基础概念  | 定义                                                                                |
| :-------- | :---------------------------------------------------------------------------------- |
| GEMM      | General Matrix Multiplication，计算矩阵乘积及可选累加的通用矩阵乘算子。             |
| Softmax   | 把一组分数经过指数归一化为和为 1 的非负权重的算子。                                 |
| Norm      | 用输入统计量对元素进行归一化和可选仿射变换的算子族，例如 LayerNorm、RMSNorm。       |
| RoPE      | Rotary Positional Embedding，按位置相关角度旋转向量维度对以注入相对位置信息的变换。 |
| Attention | 用 Q 与 K 的相似度经缩放、Mask 和 Softmax 得到权重，再对 V 加权求和的算子结构。     |
| KV Cache  | 自回归推理中保存历史 Token 的 K、V Tensor，供后续 Decode 步复用的缓存。             |
| 量化      | 用较低位宽数值及 Scale、Zero-point 等参数近似表示和计算原始高精度数据的方法。       |

#### 概念组 14：生产工程

| 基础概念 | 定义                                                                                      |
| :------- | :---------------------------------------------------------------------------------------- |
| CUDA     | NVIDIA 的 GPU 并行计算平台和编程模型，提供线程层级、内存、同步、运行时和工具链。          |
| Triton   | 用 Python 风格 DSL 描述 GPU Block 级程序，并由编译器生成目标 GPU 代码的 Kernel 开发系统。 |
| CUTLASS  | NVIDIA 提供的高性能 CUDA C++ 模板库，用可组合 Tile、Layout 和流水抽象构建矩阵类算子。     |
| Dispatch | 在运行时根据 Shape、dtype、Layout、Alignment、架构等条件选择具体实现的过程。              |
| Autotune | 在代表性输入上测量多个合法配置，并按既定目标自动选择较优配置的过程。                      |
| Fallback | 快路径条件不满足或不可用时，保证契约正确的通用、安全实现路径。                            |

#### 概念组 15：系统衔接

| 基础概念    | 定义                                                                                        |
| :---------- | :------------------------------------------------------------------------------------------ |
| Kernel 时间 | GPU 执行某个 Kernel 从设备侧开始到结束的时间，不包含未明确计入的 CPU 与其他算子成本。       |
| 同步        | 让 CPU、Stream 或设备等待指定异步工作完成，或显式建立不同工作之间先后依赖的操作。           |
| Launch      | CPU 通过 CUDA Runtime/Driver 把 Kernel 及参数提交到指定设备和 Stream 的动作及其宿主开销。   |
| 端到端延迟  | 从用户或系统定义的整体操作起点到最终结果可用所经历的全部时间。                              |
| Amdahl 上限 | 当仅加速系统中占比为 `p` 的部分且其加速比为 `s` 时，整体加速不超过 `1/((1-p)+p/s)` 的界限。 |
...
#### 双维自查详细答案：以“增大 Thread Tile”为完整示范

这组问题是一套通用作答框架，没有脱离具体优化点的唯一答案。下面用“把 GEMM Thread Tile 从 1×1 增大到 2×2”逐项示范岗位级答案。

1. **解决的问题。** 1×1 映射中，每个线程只生成一个输出，多个线程会反复从 Shared Memory 读取相同 A/B 元素。2×2 让一个线程保存 4 个累加器，期望用较少的 Shared Load 服务更多 FMA，提高寄存器复用和指令级并行。
2. **定义和作用对象。** Thread Tile 是一个线程负责的输出子块。2×2 表示每个 K 步读取 2 个 A 值和 2 个 B 值，在寄存器中更新 4 个输出；它改变的是线程到输出的映射、Shared 读取和寄存器需求，不改变数学上的 `2MNK` Effective FLOPs。
3. **上游控制变量。** `TM/TN`、CTA Tile、Block 线程布局、M/N/K Shape、边界策略和编译器寄存器分配共同决定结果。只改 `TM/TN` 时仍要保证 CTA 覆盖同一输出区域，避免同时改变 Block 大小造成无法归因的实验。
4. **到 Duration 的因果链。** `Thread Tile↑ → 每个输出平均 Shared Load↓、ILP可能↑ → registers/thread↑、Block线程数/Grid/Wave也可能变化 → Blocks/SM与Eligible Warp变化 → Shared/依赖/资源 Stall变化 → Duration`。收益和代价必须同时追踪。
5. **竞争假设与证伪。** 若 2×2 更快，假设一是 Shared 指令减少，可比较相关指令/吞吐；假设二是 ILP 提升，可观察 Eligible Warp 和依赖 Stall。若变慢，可能是寄存器限制驻留或 Spill，也可能是 Grid 变小造成 Wave Tail；前者看资源/Local Load Store，后者看 Launch Stats 和不同 M/N Shape。
6. **证据层次。** CUDA Event/NCU Duration 判定快慢；编译报告核对 registers/thread；NCU 看 Launch、Occupancy、Eligible Warp、Stall、Shared 指令与 Local Memory；有必要时看 SASS 确认 Load/FMA 数量。任何诊断指标都不能替代 Duration。
7. **代价和边界。** 小 M/N、不规则边界、寄存器紧张或低复用算子上可能不划算；边界必须 Predicate 或走 Fallback。回归用例至少覆盖典型方阵、小 M、非 Tile 整数倍和一个曾退化的 Shape，同时验证精度和性能阈值。

换成 Padding、Vectorization、Pipeline 或 Tensor Core 时，答案仍应覆盖这七个位置；无法写出其中一个位置，就说明概念之间的关系还没有打通。
...
1. **冻结同一任务。** 所有实现必须使用相同数学公式、输入数据、Shape、dtype、累加/输出精度、布局和错误语义。若一个版本偷偷用 TF32、忽略尾部、预先转置却不计成本或少写结果，它即使 Duration 更短也不是同任务优化。契约应先写成测试，再写 Kernel。
2. **Shape Matrix 的四类输入。** 真实 Shape 代表目标工作负载；边界 Shape 覆盖 0/1、Warp/Tile 前后；不规则 Shape 检查 M/N/K 尾部和 Alignment；压力 Shape 验证大规模、资源和稳定性。至少保留方阵、小 M、长条和非整齐输入，防止只为一个整齐方阵调优。
3. **一次只改一个主要变量。** 实验前写“改变什么、预计哪些中间指标怎样变、Duration 方向、可能代价”。若同时改变 Tile、Block线程数、dtype 和边界策略，即使性能变化也无法归因。确实需要组合优化时，先完成单变量实验，再把组合版本作为新的对照。
4. **三层 Baseline。** Naive 用于解释相对改动；PyTorch/cuBLAS 代表生产级供应商库差距；当前最佳版本防止后续优化相对较弱基线显得成功。数学 Reference 则负责小 Shape 高精度正确性。报告要同时保留，不用“比 Naive 快”冒充接近库性能。
5. **统计而非最好一次。** Warmup 后收集足够重复样本，报告 Median 以及 P90/P95 或离散程度，并固定环境、同步和输入。最小值只代表最幸运的一次，容易隐藏调度、频率或缓存波动；异常点需要解释来源，不能为美化结果任意删除。
...
1. **从 M/N/K 推导。** 对 `C[M,N]=A[M,K]×B[K,N]`，线程负责 `C[m,n]` 时读取 `A[m,k]` 与 `B[k,n]`，地址分别是 `m*K+k` 和 `k*N+n`，写 `m*N+n`。有效 FLOPs 常按每个 K 一乘一加计为 `2MNK`。FP32 在理想完全复用下最低 Global bytes 约为 `4(MK+KN+MN)`；Naive 无跨线程显式复用时，代码级读取上界接近 `8MNK` 加输出 `4MN`。
2. **三种“读取量”不同。** 源码 Load 次数描述线程执行多少条读取；Cache Request/Sector 描述这些地址怎样合并并命中 L1/L2；DRAM bytes 只统计最终穿过显存接口的数据。广播、合并和缓存复用会让它们差很多，所以不能用源码 `2MNK` 次读取直接宣称 DRAM 一定搬了同等字节。
3. **A/B 的 Warp 访问。** 若 `threadIdx.x` 映射 n、同一 Warp 固定 m，则某个 K 步中各 Lane 对 `A[m,k]` 访问相同地址，可能由广播/Cache 复用；对 `B[k,n]` 访问相邻地址，通常容易合并。若线程映射互换，两者模式也随之改变。必须依据实际 Warp Lane 到 m/n 的映射分析，不能只看 A、B 是 Row-major。
4. **先模型后 Profiler。** 模型强迫你明确工作量、地址和预期方向，让指标有问题可回答；Profiler 用于修正 Cache、事务、指令和调度等理想模型未覆盖的现实。如果先看上百个指标再编故事，很容易把相关性当因果。正确循环是“模型提出可证伪预测 → 实测发现差异 → Profiler定位模型漏项 → 更新模型”。
...
1. **画出三层循环。** CTA Tile 是一个 Block 负责的 `BM×BN` 输出区域；线程或 Warp 在其中负责若干输出；K 维按 `BK` 分段循环。每轮把 A 的 `BM×BK` 和 B 的 `BK×BN` Tile 从 Global 搬到 Shared，协作计算局部乘加，最后把累加器写回对应 C 地址。
2. **推导读取减少。** Naive 中一个 `BM×BN` 输出 Tile 对每个 K 近似读取 `BM×BN` 个 A 和同量 B；Tiled 版本每个 K Tile 只需读取 `BM×BK + BK×BN`，然后在 CTA 内复用。对 `BM=BN=T` 的理想方块，Global 读取量相对 Naive 约下降 T 倍；实际倍数受 Cache、边界、加载冗余和 Tile 利用率影响，必须测实际流量。
3. **两个 Barrier。** Load 后的 Barrier 保证所有线程完成 Shared Tile 写入，任何线程才开始读并计算；计算后的 Barrier 保证所有线程不再读取当前 Tile，下一轮才覆盖同一 Shared 缓冲区。缺少前者会读到未写完数据，缺少后者会让快线程覆盖慢线程仍在使用的数据。
4. **非整齐 M/N/K。** M/N 越界的 Global Load 用 Predicate 并装入 0，输出 Store 只写合法坐标；K 尾部不足 BK 时也装入 0，让乘加数学不变。所有线程仍参加 CTA Barrier，不能让越界线程提前 Return。可另外 Dispatch 整齐快路径，但通用或 Fallback 路径必须正确。
5. **Padding 是否有效。** 写出 Shared 的线性地址与 leading dimension，再按同一 Warp 同一指令计算 Bank。只有 Padding 改变了冲突访问维度的 `address % bank_count` 映射，才可能降低冲突；若原访问已无冲突，Padding 只会增加 Shared 占用、改变对齐甚至降低 Blocks/SM。用 Bank Conflicts/Wavefronts 和 Duration共同验证。
...
1. **Thread Tile 减少 Shared 读取。** 1×1 的 4 个线程生成 4 个输出，在一个 K 步可能合计读取 4 个 A 和 4 个 B；一个 2×2 线程生成同样 4 个输出，只需读取 2 个 A 和 2 个 B 并交叉更新 4 个累加器。理想下每输出 Shared Load 减少，但寄存器累加器和索引工作增加。
2. **映射与地址。** 先画 CTA 覆盖的 `BM×BN` C 区域，再将线程 `(tx,ty)` 映射到左上角 `(m0,n0)`，其 2×2 输出为 `(m0+i,n0+j)`；每个 K 步加载 A 的两个行值和 B 的两个列值，最后按 Row-major `C[(m0+i)*N+n0+j]` 写回。边界 Predicate 要覆盖每个子输出，不能只检查左上角。
3. **资源链但不以 Occupancy 结论。** 累加器增多使 registers/thread 上升，进而可能降低 Blocks/SM 和 Active Warp；随后要看 Eligible Warp、依赖/内存 Stall、是否 Spill 以及 Shared 指令是否下降。即使 Occupancy 下降，只要复用和 ILP 收益更大，Duration 仍可下降；反之 Occupancy 不变也可能因指令增多而变慢。
4. **竞争假设和最小实验。** 4×4 比 2×2 慢时可提出：寄存器限制/Spill、Grid 减小造成 Wave Tail、边界无效工作增加、索引指令增多、ILP 未被利用。每次只选一个变量：固定 CTA 输出大小比较资源，换大/小 Shape 检查 Wave，选整齐 Shape 排除边界，检查编译报告与 Local Load/Store 验证 Spill。最终回到同协议 Duration。
...
1. **从地址公式判断 Padding。** 对 Shared `tile[row][col]`，线性元素地址为 `row*LD+col`；常见 FP32 Bank 近似为该值 `%32`。把同一 Warp 同一指令的 row/col 代入：若多个 Lane 落到同一 Bank 的不同地址，Padding 改 LD 后可能打散；若访问沿连续 col，本来通常无冲突，盲目 Padding 没有收益。
2. **Vectorization 改变什么。** 宽 Load/Store 可以让每个线程用更少指令搬多个连续元素，并可能改善指令发射开销；它不自动减少算法最低 bytes，也不保证更少 Sector，因为 Warp 地址分布、对齐和缓存仍决定事务。要分别检查 SASS 指令宽度/数量、Request/Sector、实际 bytes 和 Duration。
3. **Alignment 与 Tail Fallback。** 快路径检查指针和每行起始地址满足向量宽度对齐、布局连续且主体长度可按向量宽度处理；剩余元素用 Predicate 标量尾部，条件不满足则走完整标量/通用路径。测试覆盖偏移切片、K/N 余数、空输入和多行不同起始对齐。
4. **Tile 有效利用率。** 对输出 `M×N` 和 Tile `BM×BN`，总启动槽位为 `ceil(M/BM)*BM * ceil(N/BN)*BN`，输出利用率为 `MN/总槽位`；若还考虑 K Tile，应把 `ceil(K/BK)*BK` 的无效乘加纳入执行工作量。利用率低预示 Predicate、无效线程和 Wave Tail 成本，但最终仍需实测。
5. **Dispatch 保持契约。** Dispatcher 只能根据 Shape、dtype、layout、Alignment、架构等显式条件选择实现，所有路径必须产生相同输出 Shape、定义和允许误差；若需要转换/复制，要公开并计入正确的计时范围。每条快路径都有约束测试，每个不满足条件的输入都能安全落到 Fallback，不能静默截断或降低精度。
...
1. **CTA/Warp/MMA Tile。** CTA Tile 是整个 Block 负责的输出块；它分给多个 Warp 的 Warp Tile；每个 Warp 再用若干硬件 MMA 指令覆盖更小的 MMA Tile。三层关系是包含和协作关系，不是三个可互换名字；K 维还按阶段装载并累加。
2. **Fragment、Layout 和一致执行。** Fragment 是每个 Warp 各 Lane 分布式持有的矩阵片段，不能把它当普通二维数组随意索引；Layout/Leading Dimension 告诉加载指令如何解释内存；Warp 级 MMA 要求参与 Lane 按规定共同执行。地址、对齐或控制流不满足约束可能导致错误、降级或无法生成目标指令。
3. **FP16/BF16/TF32。** FP16 范围较窄、尾数较多；BF16 范围接近 FP32、尾数较少；TF32 通常以 FP32 数据进入受支持矩阵路径，但乘法有效精度降低。常见高性能路径以低精度乘、FP32 累加，但最终误差还受输入量化、K 长度和输出转换影响。必须由接口明确是否允许 TF32 及容差。
4. **证明 Tensor Core 路径。** 先确认 GPU、dtype、Shape、Layout、Alignment 和库/编译配置满足约束；再用 NCU 的 Tensor/MMA Pipe 指标或反汇编中的目标 MMA 指令确认；最后比较 Duration 与数值结果。源码使用 WMMA API 或输入为 FP16 本身都不是充分证据。
5. **Fallback 与成本。** 非整齐 M/N/K 可用 Predication、Padding 临时缓冲、分块处理或通用 SIMT/库路径。Dispatcher 必须覆盖每个不满足快路径约束的输入；若 Padding/复制/转换发生，其时间和额外 bytes 要计入相应端到端成本，并用边界 Shape 做正确性与性能回归。
...
1. **NSYS 后 NCU。** NSYS 先回答时间落在 CPU、Launch、Memcpy、同步、Kernel 还是多个 Kernel 间隙，避免对并非主要耗时的 Kernel 做深挖；确认目标 Kernel 后，NCU 才回答其内部访存、计算、资源和调度问题。二者是层级关系，不是互相替代。
2. **NCU 八步。** 按 `Duration → Launch Stats → SM vs Memory → DRAM → Occupancy → Scheduler/Stall → 条件式 Roofline → 回到 Duration`。先确认结果和工作量相同，再检查 Grid/Block/Wave、宏观利用、实际显存流量、驻留资源和可发射状态。Roofline 只在 FLOPs/bytes 定义可靠时使用，不能机械地作为最后裁决。
3. **解释 Stall。** Stall 百分比只说明采样到的等待类别，先看每 Scheduler 的 Active/Eligible Warp 和 Issue Slot；再把主要 Stall 对应到源码依赖、Load、Barrier 或资源变化，并设计单变量实验。Eligible 足够却 Issue 低与 Eligible 本身不足是不同问题，处理方法不同。
4. **Roofline 定义。** 明确 FLOPs 是 Effective 还是 Executed、dtype 对应哪个峰值、bytes 是最低算法流量还是哪一层实测流量，并使用同一时间区间。`AI=FLOPs/bytes` 与硬件峰值算力/带宽给出上限；它是宏观分类模型，不解释 Bank Conflict、依赖链或小 Grid。
5. **回到结果。** Occupancy、Throughput、Stall、Sector 等都是诊断量。只有在正确性、工作量和协议一致前提下，固定 Shape 的 CUDA Event/NCU Duration 下降或派生吞吐提高，才能说优化成功；并要声明代价、波动和适用边界。
...
1. **抽象层次。** CUDA C++ 直接编写线程、地址、Shared 和同步；WMMA 是 CUDA 中较薄的 Warp 矩阵接口；CUTLASS/CuTe 提供可组合的高性能矩阵计算模板与 Layout/Tile 抽象；Triton 用 Python 风格 DSL 和编译器表达 Block 级程序。层次更高不等于不需要理解硬件，区别主要是由谁负责映射、调度和代码生成。
2. **真正掌握 Triton。** 能解释 Program ID 到输出 Tile、Mask 到边界、Block Pointer/地址到 Layout，并独立修改 `BLOCK_M/N/K`、`num_warps`、`num_stages` 或 Grouping；修改前预测复用、并行度和资源方向，修改后用同协议测试与 profiler 验证。只运行官方示例不构成掌握。
3. **JIT/Autotune 与稳态分开。** 首次调用可能包含编译、代码缓存和 Autotune 多配置搜索，回答“首次可用延迟”；缓存后的重复调用回答稳态 Kernel 性能。两者分别计时和报告，不能把预热成本偷偷排除后声称端到端延迟，也不能把首次编译混进 Kernel Duration。
4. **公平对照。** CUDA、Triton、CUTLASS/cuBLAS 使用相同输入、Shape、dtype、TF32设置、累加/输出精度、Stream、Warmup 和统计方式；任何预处理、布局转换和 Workspace 要说明是否计时。报告同时保留绝对 Duration、相对库性能和失败/退化 Shape，诚实解释教学实现与生产库的功能差距。
...
1. **Max trick 与 Online 合并。** 普通稳定 Softmax 先求 `m=max(x)`，再算 `l=Σexp(x-m)`。两个分块状态 `(m1,l1)`、`(m2,l2)` 合并为 `m=max(m1,m2)`，`l=l1*exp(m1-m)+l2*exp(m2-m)`；因为两部分都被重标定到共同最大值 m，等价于对并集求和。该合并满足分块归约需要的结合结构，可逐段扫描而保持数值稳定。
2. **三种实现读写。** 朴素组合式实现可能分别做 Max、Exp/Sum、Normalize，多次从 Global 读取并写中间 Tensor；片上 Fused 版本通常读输入一次、在 Register/Shared 中完成统计并写输出一次；Online 版本可在一次扫描中更新 `(m,l)`，但若无法把整行元素保存在片上，为最终输出仍可能需要重读输入。必须按具体代码和是否物化中间值计数，不能只按算法名字下结论。
3. **Rows/Cols 映射。** 短行可一 Warp 一行，用 Shuffle 归约；中等行常一 Block 一行，用多个 Warp 和 Shared 合并；超长行可能需要每线程多元素、分块或多阶段策略。选择取决于 Cols、dtype、寄存器/Shared 容量和 Rows 并行度；测试 1、31、32、33、非 2 次幂和长行边界。
4. **Mask/Scale 融合的收益边界。** 把 Scale、Mask 与 Softmax 放在同一 Kernel 可减少独立 Kernel Launch 和中间 Tensor 的 Global 读写；它不会减少 Softmax 数学上必需的比较、指数、求和和输出，也可能增加寄存器或分支。必须比较完整等价序列的端到端时间，而不是只拿融合 Kernel 与其中一个旧 Kernel 比。
5. **有效 GB/s 与实际 DRAM bytes。** 有效 GB/s 用算法约定的最低有效读写字节除以 Duration，便于同任务版本比较；实际 DRAM bytes 来自硬件计数，包含重复访问、事务浪费并受 Cache 影响。前者是结果指标，后者是诊断证据；二者差异能提示实现流量效率，但不能混用同一名称。
